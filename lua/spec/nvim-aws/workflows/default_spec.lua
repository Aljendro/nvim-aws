...
describe("testing default workflow", function()
	...
	it("should return empty table for invalid service", function()
		local commands = default_workflow.get_service_commands("non_existent_service")
		assert.are_same({}, commands)
	end)

	it("should create a template buffer correctly", function()
		local buf = default_workflow.create_template_buffer("s3api", "list_buckets", "{}")

		-- Check buffer properties
		assert.is_true(vim.api.nvim_buf_is_valid(buf))
		local name = vim.api.nvim_buf_get_name(buf)
		assert.are_equal("json", vim.api.nvim_buf_get_option(buf, "filetype"))
		assert.are_equal("wipe", vim.api.nvim_buf_get_option(buf, "bufhidden"))
	...
	it("should execute valid commands for available services", function()
		local result = default_workflow.execute_command("cloudwatch", "list_metrics", '{"Namespace": "AWS/EC2"}')
	...
	it("should handle invalid service in execute_command", function()
		local result = default_workflow.execute_command("non_existent_service", "any_command", "{}")
	...
	it("should handle invalid command in execute_command", function()
		local result = default_workflow.execute_command("s3api", "non_existent_command", "{}")
		assert.is_false(result.success)
		assert.is_string(result.error)
	...
	it("should handle invalid JSON in execute_command", function()
		local result = default_workflow.execute_command("s3api", "list_buckets", "invalid json")
		assert.is_false(result.success)
		assert.is_string(result.error)
